...
export default class Order extends React.Component {
  ...
  renderOrderState () {
    switch (this.state.order.workflow_state) {
      case 'not_confirm':
        return (
          <Text style={styles.state}><Icon name="question-circle" size={16}/> 未确认</Text>
        )
        break;
      case 'awaiting_upload':
        return (
          <Text style={styles.state}><Icon name="clock-o" size={16}/> 待定稿</Text>
        )
        break;
      case 'awaiting_review':
        return (
          <Text style={styles.state}><Icon name="search" size={16}/> 审核中</Text>
        )
        break;
      case 'typesetting':
        return (
          <Text style={styles.state}><Icon name="files-o" size={16}/> 排版中</Text>
        )
        break;
      case 'deadline':
        return (
    ...
    return (
      <View style={[styles.order, borderStyle]}>
        <View style={styles.title}>
          <TouchableHighlight onPress={() => {if(this.props.onNamePress)this.props.onNamePress()}} underlayColor='#ccc'><Text style={[styles.fontSize18, styles.fontWeightBold, styles.fontColorBlue]}>{this.state.order.product_name}</Text></TouchableHighlight>
          <Text style={[styles.fontSize18, styles.fontColorBlue]}>{this.state.order.price}</Text>
        </View>
        <View style={styles.idAndState}>
          <TouchableHighlight onLongPress={()=> this.setClipboardContent(this.state.order.id)} onPress={() => {if(this.props.onNamePress)this.props.onNamePress()}} underlayColor='#ccc'>
          <Text style={styles.id}>{this.state.order.id}</Text>
          </TouchableHighlight>
          {this.renderOrderState()}
        </View>
        <View><TouchableHighlight onPress={() => {if(this.props.onNamePress){this.props.onNamePress.bind(this)}}} underlayColor='#ccc'><Text>{this.state.order.user.name}({this.state.order.user.fake_name})</Text></TouchableHighlight></View>
